...
func TestMySQL_init(t *testing.T) {
	...
	script := db.build()
	assert.Equal(t, script, "mysqldump --host 1.2.3.4 --port 1234 -u user1 --ignore-table=my_db.aa --ignore-table=my_db.bb --a1 --a2 --a3 my_db foo bar --result-file=/data/backups/mysql/mysql1/my_db.sql")
}
...
func TestMySQL_dumpArgsWithAdditionalOptions(t *testing.T) {
	...
	assert.Equal(t, db.build(), "mysqldump --host 127.0.0.2 --port 6378 --single-transaction --quick dummy_test --result-file=/data/backups/mysql/mysql1/dummy_test.sql")
}
...
func TestMySQL_dumpArgsWithDollarInPassword(t *testing.T) {
	...
	// The password (with special characters) is passed as a discrete -p argument at
	// exec time, so it must never appear in the command line built here.
