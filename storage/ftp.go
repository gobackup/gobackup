...
func (s *FTP) uploadState(key string, data []byte) error {
	...
	if err := s.client.Stor(remotePath, reader); err != nil {
		return fmt.Errorf("failed to store state file: %v", err)
	}
...
func (s *FTP) downloadState(key string) ([]byte, error) {
	remotePath := path.Join(s.path, key)

	resp, err := s.client.Retr(remotePath)
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve state file: %v", err)
	}
	defer resp.Close()
