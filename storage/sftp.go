...
func (s *SFTP) uploadState(key string, data []byte) error {
	...
	if err != nil {
		return fmt.Errorf("failed to open state file for writing: %v", err)
	}
	defer file.Close()
...
func (s *SFTP) downloadState(key string) ([]byte, error) {
	remotePath := path.Join(s.path, key)
